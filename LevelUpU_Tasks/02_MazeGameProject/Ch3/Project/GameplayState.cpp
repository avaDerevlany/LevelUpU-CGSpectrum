	...
	, m_pLevel(nullptr)
{
	...
	//m_currentLevel = GameLevels::GetInstance()->GetCurrentLevel();
}
...
bool GameplayState::Load()
{
	...
	return m_pLevel->Load(GameLevels::GetInstance()->GetCurrentLevelName(), m_player.GetXPositionPointer(), m_player.GetYPositionPointer());
...
bool GameplayState::Update()
{
	if (m_beatLevel)
	{
		...
		{
			m_player.RemoveSword();	// sword only lasts for the level

			m_beatLevel = false;
			m_skipFrameCount = 0;
			GameLevels::GetInstance()->IncreaseCurrentLevel();
			//++m_currentLevel;
			if (GameLevels::GetInstance()->CheckReachedMaxLevel())
			{
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		case ActorType::Sword:
		{
			...
			if (!m_player.HasSword())
			{
				...
				m_player.SetPosition(newPlayerX, newPlayerY);
				AudioManager::GetInstance()->PlayKeyPickupSound();	// AudioManager::GetInstance()->PlaySwordPickupSound();
			}
			break;
		}
		case ActorType::Door:
...
void GameplayState::DrawHUD(const HANDLE& console)
{
	...
	cout << Level::WAL << " sword:";
	if (m_player.HasSword())
	{
		m_player.GetSword()->Draw();
	}
	else
	{
		cout << " ";
